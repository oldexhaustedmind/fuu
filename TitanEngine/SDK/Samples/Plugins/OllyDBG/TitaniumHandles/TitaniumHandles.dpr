...
resourcestring
  ...
  PLUGIN_NAME = 'TitaniumHandles';
  ABOUT   = 'Plugin coded by ReversingLabs Corporation' + #13#10 + 'Version: 1.0.0' + #13#10 + 'Web: http://www.reversinglabs.com';
...
begin
  ...
  g_hwndOlly := hWndOlly;
  Addtolist(0, 0, 'TitaniumHandles - Ver %s by %s', VERSION, AUTHOR);
  ...
  case origin of
    PM_MAIN:
      begin
        ...
        StrCopy(pData, '0 &TitaniumHandles, 1 &About...');
        Result := 1;
      end;
  else
  ...
  begin
    sExePath := GetExePath;
     ...
     if action = 1 then begin
      Infoline(PChar(' TitaniumHandles version: ' + VERSION + ' coded by ' + AUTHOR));
