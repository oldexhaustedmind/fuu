...
-- Variables
	...
	unpackerOptionRealignFile = TE_FALSE
	unpackerOptionCopyOverlay = TE_FALSE
...
local function _TE_cbMakeSnapShoot1()
	...
	if TE_RelocaterMakeSnapshot(ProcessInfo.hProcess, "snapshot.1", vSnapshootMemoryStartRVA + vFileBase, vSnapshootMemorySize) == TE_TRUE then
		TE_EngineAddUnpackerWindowLogMessage("[x] Created memory snapshot #1 from ".. string.format("%08X",vSnapshootMemoryStartRVA + vFileBase) .. " size " .. string.format("%08X",vSnapshootMemorySize))
	else
		TE_EngineAddUnpackerWindowLogMessage("Critical error, aborting...")
		TE_StopDebug()
	end
end
...
local function _TE_cbLoadLibrary()
	...
	if cAddress == fUPXSignature1 then
		if TE_GetRemoteString(ProcessInfo.hProcess, TE_GetContextData(UE_EAX), RemoteString, MAX_PATH) == TE_TRUE then
			...
			TE_EngineAddUnpackerWindowLogMessage("[x] LoadLibrary BPX -> " .. szRemoteString)
			TE_ImporterAddNewDll(szRemoteString, nil)
		else
			TE_EngineAddUnpackerWindowLogMessage("Could not read remote string, aborting...")
			TE_StopDebug()
		end
	else
		TE_EngineAddUnpackerWindowLogMessage("Critical error, aborting...")
	...
	if not(cTargetAddress == 0) then
		if cTargetAddress > vFileBase then
			if TE_GetRemoteString(ProcessInfo.hProcess, cTargetAddress, RemoteString, MAX_PATH) == TE_TRUE then
				szRemoteString = tostring(RemoteString)
				TE_EngineAddUnpackerWindowLogMessage("[x] GetProcAddress (0x" .. string.format("%08X",cThunk) .. ") BPX -> " .. szRemoteString)
			...
			TE_EngineAddUnpackerWindowLogMessage("[x] GetProcAddress (0x" .. string.format("%08X",cThunk) .. ") BPX -> " .. string.format("%08X",cTargetAddress))
...
local function _TE_cbEntryPoint()
	...
	if vFileIsDLL == TE_FALSE then
		...
		if TE_RelocaterMakeSnapshot(ProcessInfo.hProcess, "snapshot.2", vSnapshootMemoryStartRVA + vFileBase, vSnapshootMemorySize) == TE_TRUE then
			TE_EngineAddUnpackerWindowLogMessage("[x] Created memory snapshot #2 from ".. string.format("%08X",vSnapshootMemoryStartRVA + vFileBase) .. " size " .. string.format("%08X",vSnapshootMemorySize))
			if TE_RelocaterCompareTwoSnapshots(ProcessInfo.hProcess, vFileBase, vSizeOfImage, "snapshot.1", "snapshot.2", vSnapshootMemoryStartRVA + vFileBase) == TE_FALSE then
				...
				TE_EngineAddUnpackerWindowLogMessage("[x] Compared snapshots")
	...
	TE_EngineAddUnpackerWindowLogMessage("[x] Entry Point at: " .. string.format("%08x",vUnpackedOEP))
	if TE_DumpProcess(ProcessInfo.hProcess, vFileBase, fOutName, vUnpackedOEP) == TE_TRUE then
		TE_EngineAddUnpackerWindowLogMessage("[x] Process dumped!")
		...
		if TE_StaticFileLoad(fOutName, UE_ACCESS_ALL, TE_FALSE, pFileHandle, pLoadedSize, pFileMap, pFileMapVA) then
			...
			if TE_ImporterExportIAT(TE_ConvertVAtoFileOffset(FileMapVA, mImportTableOffset, TE_TRUE), FileMapVA) == TE_FALSE then
				TE_EngineAddUnpackerWindowLogMessage("IAT: Critical error, aborting...")
			else
				TE_EngineAddUnpackerWindowLogMessage("[x] IAT has been fixed!")
			end
			if vFileIsDLL == TE_TRUE then
				if TE_RelocaterExportRelocation(TE_ConvertVAtoFileOffset(FileMapVA, mRelocTableOffset + vFileBase, TE_TRUE), mRelocTableOffset, FileMapVA) == TE_FALSE then
					TE_EngineAddUnpackerWindowLogMessage("RELOCATE: Critical error, aborting...")
				else
					TE_EngineAddUnpackerWindowLogMessage("[x] Exporting relocations!")
				end
			end
			if unpackerOptionRealignFile == TE_TRUE then
				LoadedSize = TE_RealignPE(FileMapVA, LoadedSize, 0)
				TE_EngineAddUnpackerWindowLogMessage("[x] Realigning file!")
			end
			TE_StaticFileUnload(fOutName, TE_TRUE, FileHandle, LoadedSize, FileMap, FileMapVA)
			TE_MakeAllSectionsRWE(fOutName)
			if unpackerOptionCopyOverlay == TE_TRUE then
				if TE_CopyOverlay(fname, fOutName) == TE_TRUE then
					TE_EngineAddUnpackerWindowLogMessage("[x] Moved overlay!")
				end
			end
			TE_EngineAddUnpackerWindowLogMessage("-> Unpack ended...")
			TE_StopDebug()
		end
	else
		TE_EngineAddUnpackerWindowLogMessage("Critical error, aborting...")
...
local function _TE_InitializeDebug()
	...
	TE_EngineAddUnpackerWindowLogMessage("Locating patterns...")
	...
	if vFileIsDLL == TE_TRUE then
		...
		TE_ImporterInit(50 * 1024, vImageBase);
	end
	vCalculatedSearchSize = TE_GetPE32Data(fname, 0, UE_SIZEOFIMAGE) - vPackedOEP
	...
	if fUPXSignature1 == 0 then
		TE_EngineAddUnpackerWindowLogMessage("Could not find search pattern #1, aborting...")
		vResult = TE_FALSE
		TE_StopDebug()
	end
	...
	if fUPXSignature1 == 0 then
		TE_EngineAddUnpackerWindowLogMessage("Could not find search pattern #3, aborting...")
		vResult = TE_FALSE
		TE_StopDebug()
	end
	...
	if fUPXSignature5 == 0 then
		...
		if fUPXSignature5 == 0 then
			TE_EngineAddUnpackerWindowLogMessage("Could not find search pattern #5, aborting...")
			vResult = TE_FALSE
			TE_StopDebug()
		else
	...
	if vFileIsDLL == TE_TRUE then
		...
		if fUPXSignature6 == 0 then
			TE_EngineAddUnpackerWindowLogMessage("Could not find search pattern #6, aborting...")
			vResult = TE_FALSE
			TE_StopDebug()
		else
	...
	if vResult == TE_TRUE then
		TE_EngineAddUnpackerWindowLogMessage("All patterns found...")
		TE_EngineAddUnpackerWindowLogMessage("[x] Setting breakpoints...")
		...
		TE_EngineAddUnpackerWindowLogMessage("[Error] File is not packed with UPX 1.x - 3.x")
	end
end
TE_InitializeDebug_CB = alien.callback(_TE_InitializeDebug, "void")

local function _TE_InitializeDBG(fOriginalName, RealignFile, CopyOverlay)


	fname = fOriginalName
	unpackerOptionRealignFile = RealignFile
	unpackerOptionCopyOverlay = CopyOverlay
	fOutName = string.sub(fOriginalName, 1, string.len(fOriginalName) - 4) .. "_unpacked" .. string.sub(fOriginalName, string.len(fOriginalName) - 3)
	TE_EngineAddUnpackerWindowLogMessage("Unpacker for UPX 1.x - 3.x packed files")
	TE_EngineAddUnpackerWindowLogMessage("ReversingLabs corporation / www.reversinglabs.com")
	if not(fname == nil) then
		...
		if not(pProcessInfo == nil) then
			TE_EngineAddUnpackerWindowLogMessage("Starting debugging...")
			...
			TE_DebugLoop()
			TE_EngineAddUnpackerWindowLogMessage("[x] ExitCode: " .. string.format("%08x",TE_GetExitCode()))
		else
			TE_EngineAddUnpackerWindowLogMessage("Could not initialize debugging!")
		end
	end
end
TE_InitializeDBG = alien.callback(_TE_InitializeDBG, "void", "string", "byte", "byte")
...
--
	...
	TE_EngineCreateUnpackerWindow("[ RL!deUPX ]", "RL!deUPX 1.x - 3.x", "RL!deUPX 1.x - 3.x", "ReversingLabs Corporation", TE_InitializeDBG)
