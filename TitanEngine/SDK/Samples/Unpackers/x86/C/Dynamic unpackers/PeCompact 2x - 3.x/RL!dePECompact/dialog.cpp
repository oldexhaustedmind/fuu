...
int __stdcall cDialog::Handler(HWND pDialog, int pMessage, int pwParam, int plParam)
{
	...
	case WM_COMMAND:
		...
		case rlUnpack:
			...
			Unpack.Unpack(lFile, IsDlgButtonChecked(pDialog, rlRealign) == BST_CHECKED, IsDlgButtonChecked(pDialog, rlOverlay) == BST_CHECKED);
