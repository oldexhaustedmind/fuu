...
InitializeUnpacker proc szFileName:DWORD,dwCallBack:DWORD
	...
	.if EAX != NULL && BYTE PTR[EAX] != NULL
		INVOKE IsPE32FileValidEx,szFileName,UE_DEPTH_DEEP,NULL
		.if EAX == 1
			INVOKE GetPE32Data,szFileName,NULL,UE_IMAGEBASE
			MOV fdImageBase,EAX
			INVOKE GetPE32Data,szFileName,NULL,UE_OEP
			MOV fdEntryPoint,EAX
			INVOKE GetPE32Data,szFileName,NULL,UE_SIZEOFIMAGE
			...
			INVOKE GetPE32Data,szFileName,NULL,UE_SECTIONVIRTUALOFFSET
			...
			INVOKE IsFileDLL,szFileName,NULL
			MOV fdFileIsDll,AL
			.if fdFileIsDll == NULL
			...
			.if EAX != NULL
				...
				.if fdFileIsDll == NULL
					INVOKE SetCustomHandler,UE_CH_CREATEPROCESS,addr cbCreateProcess
				.endif
				INVOKE DebugLoop
			.else
...
cbEntryPoint proc
	...
	INVOKE AddLogMessage,addr GlobalTempBuffer
	.if fdFileIsDll != 1
		INVOKE PastePEHeader,DWORD PTR[fdProcessInfo.hProcess],fdLoadedBase,addr GlobalBuffer
		INVOKE AddLogMessage,addr szPastePEHeader
	.else
		MOV ESI,SnapshootMemoryStartRVA
		ADD ESI,fdLoadedBase
		.if dtSecondSnapShootOnEP == 1
			INVOKE RelocaterMakeSnapshot,DWORD PTR[fdProcessInfo.hProcess],addr SnapShoot2,ESI,SnapshootMemorySize
		.endif
	...
	.if rsFileMapVA > NULL
		INVOKE ConvertVAtoFileOffset,rsFileMapVA,EBX,1
		MOV EBX,EAX
...
cbMakeSnapShoot1 proc
	...
	MOV ESI,SnapshootMemoryStartRVA
	ADD ESI,fdLoadedBase
	INVOKE RelocaterMakeSnapshot,DWORD PTR[fdProcessInfo.hProcess],addr SnapShoot1,ESI,SnapshootMemorySize
...
cbFindPatterns proc
	...
	.if EAX != NULL
		...
		.if EAX != NULL
			...
			INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern1CallBack
		.else
		...
		.if EAX != NULL
			...
			INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern2CallBack
		.endif
		...
		.if EAX != NULL
			...
			INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern3CallBack
		.else
			.if DontLog == NULL
				INVOKE AddLogMessage,addr szNotPackErrorText
				INVOKE AddLogMessage,addr szEndUnpack
		...
		.if EAX != NULL
			...
			INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern4CallBack
		.endif
		...
		.if EAX != NULL
			...
			INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern5CallBack
		.else
			...
			.if EAX != NULL
				...
				INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern51CallBack
			.else
				.if DontLog == NULL
					INVOKE AddLogMessage,addr szNotPackErrorText
					INVOKE AddLogMessage,addr szEndUnpack
		...
		.if fdFileIsDll == 1
			...
			.if EAX != NULL
				...
				INVOKE SetBPX,EBX,UE_BREAKPOINT,dtPattern6CallBack
			.else
				.if DontLog == NULL
					INVOKE AddLogMessage,addr szNotPackErrorText
					INVOKE AddLogMessage,addr szEndUnpack
